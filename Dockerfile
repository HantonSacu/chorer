...
FROM verybigthings/elixir:1.13 AS build
...
COPY --chown=${APP_USER} --from=pre-release /opt/cache/_build/prod/rel/brevity .

USER ${APP_USER}

CMD trap 'exit' INT; ${WORKDIR}/bin/brevity start
